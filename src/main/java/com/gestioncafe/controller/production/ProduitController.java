...
public class ProduitController {
    ...
    public String listProduits(Model model) {
        ...
        for (Produit produit : produits) {
            PrixVenteProduit prix = prixVenteProduitService.findLastByProduitId(Math.toIntExact(produit.getId()));
            prixVenteMap.put(Math.toIntExact(produit.getId()), prix != null ? prix.getPrixVente() : BigDecimal.ZERO);
        }
    ...
    public String saveProduit(
        ...
        Recette recette;
        List<Recette> recettesExistantes = recetteService.findByProduitId(Math.toIntExact(savedProduit.getId()));
        ...
        for (IngredientFormDTO ing : ingredientsList) {
            try {
                if (ing.getIdMatierePremiere() != null && ing.getIdUnite() != null && ing.getQuantite() != null) {
                    MatierePremiere mp = matierePremiereService.findById(Long.valueOf(ing.getIdMatierePremiere())).orElse(null);
        ...
        BigDecimal prixVente = produitService.calculerPrixVente(
            Math.toIntExact(savedProduit.getId()), modePrix, coefficient, prixVenteManuel
        );
        PrixVenteProduit pvp = new PrixVenteProduit();
        pvp.setProduit(savedProduit);
        pvp.setPrixVente(prixVente);
        pvp.setDateApplication(LocalDate.from(LocalDateTime.now()));
    ...
    public String showEditForm(@PathVariable Integer id, Model model) {
        ...
        if (produitOpt.isPresent()) {
            ...
            List<Recette> recettes = recetteService.findByProduitId(Math.toIntExact(produit.getId()));
            if (recettes != null && !recettes.isEmpty()) {
                ...
                List<IngredientFormDTO> ingredientDTOs = new java.util.ArrayList<>();
                ...
                for (DetailRecette detail : details) {
                    ...
                    if (estimationRecente != null && estimationRecente.getPrix() != null) {
                        ...
                        BigDecimal valeurParNormeRecette = detail.getUnite().getValeurPrNorme();
                        ...
                        BigDecimal valeurParNormeEstimation = estimationRecente.getUnite().getValeurPrNorme();
                ...
                IngredientsFormWrapper wrapper = new IngredientsFormWrapper();
                ...
                PrixVenteProduit prixVente = prixVenteProduitService.findLastByProduitId(Math.toIntExact(produit.getId()));
    ...
    public String ficheTechnique(@PathVariable Integer id, Model model) {
        ...
        if (recettes == null || recettes.isEmpty()) {
            ...
            List<DetailRecette> details = detailRecetteService.findByRecetteId(recette.getId());
            List<Map<String, Object>> detailsFiche = new ArrayList<>();
            ...
            for (DetailRecette detail : details) {
                ...
                if (estimationRecente != null && estimationRecente.getPrix() != null) {
                    BigDecimal prixEstime = BigDecimal.valueOf(estimationRecente.getPrix());
                    BigDecimal valeurParNormeEstimation = estimationRecente.getUnite() != null && estimationRecente.getUnite().getValeurPrNorme() != null ? estimationRecente.getUnite().getValeurPrNorme() : java.math.BigDecimal.ONE;
                    prixUnitaire = prixEstime.divide(valeurParNormeEstimation, 6, RoundingMode.HALF_UP);
                ...
                if (detail.getUnite() != null && detail.getUnite().getValeurPrNorme() != null) {
                    quantiteNorme = quantiteNorme.multiply(detail.getUnite().getValeurPrNorme());
                }
                map.put("quantiteNorme", quantiteNorme);

                // Prix total = prix unitaire * quantité convertie à la norme
                BigDecimal prixTotal = prixUnitaire.multiply(quantiteNorme);
